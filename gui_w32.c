...
initialise_toolbar(void)
{
	...
	s_toolbarhwnd = CreateWindowEx(WS_EX_PALETTEWINDOW, TOOLBARCLASSNAME, NULL, 
									  TBSTYLE_FLAT | WS_CHILD | TBSTYLE_WRAPABLE, 0, 0, 0, 0,
									  s_hwnd, NULL, s_hinst, NULL);

	// Create an image list
	s_toolbarImageList = ImageList_Create(TOOLBAR_BUTTON_WIDTH, TOOLBAR_BUTTON_HEIGHT, // Dimensions of individual bitmaps.
									ILC_COLOR24 | ILC_MASK, // Ensures transparent background.
									0, 0);

	s_toolbarBitmapImage = LoadBitmap(s_hinst, MAKEINTRESOURCE(IDR_TOOLBAR256));
	ImageList_AddMasked(s_toolbarImageList, s_toolbarBitmapImage, RGB(0, 255, 0));

	// Set the image list.
	SendMessage(s_toolbarhwnd, TB_SETIMAGELIST, (WPARAM)0, (LPARAM)s_toolbarImageList);

	vim_memset(&tb, 0, sizeof(tb));
    tb.fsStyle = TBSTYLE_SEP;
	tb.fsState = TBSTATE_ENABLED;
	SendMessage(s_toolbarhwnd, TB_INSERTBUTTON, (WPARAM)0, (LPARAM)&tb);
